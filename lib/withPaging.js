...
const splitNode = ({ editor, path, nodeText }) => {
  const textToExtract = Math.floor(nodeText.length / 5)
  Transforms.insertText(
    editor,
    nodeText.slice(0, nodeText.length - textToExtract),
    { at: path }
  )
  Transforms.insertNodes(
    editor,
    { type: 'paragraph', id: shortid.generate(), children: [{ text: nodeText.slice(nodeText.length - textToExtract) }] },
    { at: [path[0] + 1, 0] }
  )
